/**
 * ВСЕ привязки к разметке polemicagame.com.
 *
 * ЧТО ПРАВИТЬ ПРИ РЕДИЗАЙНЕ САЙТА — честный список (28.08.2026). Обещание
 * «правится ТОЛЬКО этот файл» было неправдой, и внешний аудит на нём поймал:
 *
 *  1. `SITE` — селекторы (с точкой) и `SITE_CLASS` — голые имена классов для
 *     `classList`. Имя игрока живёт в реестре ДВАЖДЫ: `SITE.player` = ".player"
 *     и `SITE_CLASS.player` = "player" (инвариант сверяет их согласованность).
 *  2. `src/static/notes.css` — CSS дистрибутива, в нём разметка сайта и
 *     scoped-хеш Vue (`[data-v-…]`). Список известных классов — в инварианте
 *     `architecture.test.ts`, он же чек-лист.
 *  3. CSS-БЛОКИ внутри фич: скрытие роли (auto-start), сворачивание ников
 *     (nick-plate), таблица разбора (match-stats), тултипы, порядок контролов,
 *     страница матча. Они перечислены в том же инварианте поимённо.
 *
 * Пункты 2 и 3 отваливаются МОЛЧА: правило просто перестаёт применяться,
 * исключения нет. Поэтому после редизайна проходить надо по всем трём.
 */
...
export const SITE = {
  ...
  /** Тултип наших штрафов в таблице разбора (создаём мы, имя общее). */
  penaltyTooltip: ".penalty-tooltip",
  /**
   * Атрибуты с id/данными игры (легаси-формы разметки; актуальный контракт
   * страницы разбора — `:game-data`, см. match-data.ts). Держим в реестре:
   * знание о вёрстке сайта мимо реестра — тот же дефект, что голый класс
   * (арх-аудит швов 29.08.2026, SEAM-09). ВАЖНО: страж классов атрибутные
   * селекторы НЕ видит — их согласованность держится только на этом реестре
   * (граница записана в AGENTS §6).
   */
  gameIdAttr: "[data-game-id]",
  gameDataAttrLegacy: "[data-game]",
} as const;
...
export const TEXT = {
  ...
  /**
   * ТОЧНЫЕ подписи пункта «пауза» в меню настроек игры. Держатся здесь, а не
   * внутри pause-hotkey: их читает ещё и подсказка клавиши на кнопке, а два
   * списка подписей одного и того же пункта разъехались бы при первом же
   * изменении на сайте.
   */
  pauseExact: [
    "пауза",
  ...
  /**
   * Текст стадии ДО прихода состояния комнаты (RU.js/EN.js: waiting_for_game).
   *
   * Роллер выбирает ветку по `gameView.stage.type`, а пока состояния нет,
   * `gameView` не определён — и рисуется ОБЫЧНЫЙ блок стадии с этим текстом,
   * а не экран набора игроков. Любой, кто судит «матч идёт» по непустой
   * стадии, обязан этот текст исключать: иначе свежезагруженная комната
   * читается как идущий матч (жалоба 09.08.2026 — возврат после развала не
   * сработал, потому что фича выключила себя через секунду после входа).
   */
  waitingForGameStage: ["ожидание начала игры", "waiting for game"] as readonly string[],
  guessButtons: [
    "оставить лх",
    "сбросить лх",
    "make a guess",
