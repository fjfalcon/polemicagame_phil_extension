    ...
    const rebuild = () => {
      ...
      picker.addEventListener("change", () => {
        ...
        if (c && !port.model.customTags.includes(c) && !TAG_PRESETS.some((p) => p.css === c)) {
          ...
          // Добавление — ИНТЕНТ через модель: она отправляет его координатору,
          // и цвет соседней вкладки не затирается (внешний аудит 28.08.2026).
          void port.model.addCustomTag(c).then((ok) => {
