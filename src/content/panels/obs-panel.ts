 ...
 * ВЛАДЕНИЯ автосценой. Идущий матч таких пауз не даёт: роллер обновляется
 * постоянно, а пауза/промах/итог рисуют `.ended`, который детектор ПОНИМАЕТ
 * (возвращает day без «непонимания» — счётчик не копится). Значит минута
 * непонимания = в этой вкладке матч не идёт. Закрывает дыру «та же комната,
 * новая игра»: матч кончился, комната вернулась в пре-гейм БЕЗ смены
 * pathname — сброс «живости» по комнате не срабатывал, и старая вкладка
 * держала владение всю первую ночь нового матча в соседней (лог 05.08.2026).
 */
const OWNER_PHASE_STALE_MS = 60_000;
      ...
      if (m.type === "obs_scene_owner_ping") {
        ...
        return Promise.resolve({
          owning: drivesAutoScene({
            ...
            // Свежесть: «живость» протухает после минуты непонимания фазы —
            // иначе комната, ушедшая в пре-гейм следующей игры без смены
            // pathname, держит владение вечно (лог 05.08.2026).
            phaseSeenLive:
              phaseConfirmedLive &&
