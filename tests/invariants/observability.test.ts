...
describe("терминальные исходы автовозврата (QR-1..3)", () => {
  ...
  test.each([
    ["stale-сэмпл отсчёта объявляет ожидание (RQ-1)", /отсчёт исчез после паузы вкладки/],
    ["неготовому игроку отвечают, а не молчат (RQ-7)", /готовность не была подтверждена/],
    ["неоднозначный экран ошибки — исход, не тишина (RQ-5)", /не подтверждает развал/],
    ["негодная метка на главной снимается вслух (RQ-8)", /мост автовозврата снят на главной:/],
  ])("%s", (_name, pattern) => {
    expect(source).toMatch(pattern);
  });

  test("исходы аудита 03.08 залатчены по эпизоду, не по тику", () => {
    expect(source).toMatch(/ambiguousErrorLogged/);
    expect(source).toMatch(/notReadyAnnounced/);
    expect(source).toMatch(/elsewhereStorageWarned/);
  });
